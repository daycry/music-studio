...
2. **El self-hosting no cambia el estatus legal del output.** La licencia del modelo —**MIT** en ACE-Step 1.5, **Apache 2.0** en HeartMuLa y YuE— cubre pesos y código; no limpia la procedencia de los datos de entrenamiento. Suno y Udio fueron demandados en 2025 **por los datos**, no por el modo de despliegue.
...
|---|--------|-------|---------|------------|
| **R-01** | **Procedencia legal del output.** El self-hosting no cambia el estatus legal del audio: la licencia del modelo (**MIT** en ACE-Step 1.5, **Apache 2.0** en HeartMuLa y YuE) cubre pesos y código, no los datos de entrenamiento | **Alta** | **Crítico** | **Gate G2 en la semana 0, antes de gastar** (I-05); C-10 no negociable; `training_data_declaration` obligatoria; C-09 como única vía a procedencia limpia; **§10.7 aceptado por escrito antes de aprobar** |
...
| 2026-09-03 | **Corrección de licencia: ACE-Step 1.5 es MIT, no Apache 2.0.** Las **tres** menciones de «Apache 2.0» de este documento (§2 resumen ejecutivo, §10.7 punto 2 y la fila **R-01** de riesgos) usaban «Apache 2.0» como si fuera la licencia del modelo de referencia. **Lo es de HeartMuLa y de YuE 7B, no de ACE-Step 1.5, que es MIT** — Apache 2.0 corresponde a **ACE-Step v1 3.5B**, que es otro modelo. Las tres quedan reescritas como «la licencia del modelo —MIT en ACE-Step 1.5, Apache 2.0 en HeartMuLa y YuE—», **conservando íntegro el argumento**, que no dependía de cuál de las dos licencias fuera: ninguna licencia permisiva limpia la procedencia de los datos de entrenamiento. Evidencia: `T-06` ([`spikes/comparativa-modelos.md`](./spikes/comparativa-modelos.md) §4.1) y copia archivada `LICENSE.acestep.mit.txt` (sha256 `05a6bce4…a485357`). **Sin cambio de horas, coste, fases, estados ni del veredicto de G2**: MIT y Apache 2.0 son ambas permisivas y admiten uso comercial, luego **R-01, S-05 y la matriz de G2 mantienen su valoración**. Consecuencia real: lo que se escribe en `training_data_declaration`/`license` del manifiesto de C-10, y el texto que se puso delante de legal (ver [`gates/g2-matriz-resultados.md`](./gates/g2-matriz-resultados.md) §0-bis). | dev-cycle (orquestador) |
