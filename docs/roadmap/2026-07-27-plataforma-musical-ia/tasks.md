...
4. **Integridad de pesos apagada por defecto**: `generar.cmd` no exportaba `ACE_STEP_WEIGHTS_SHA256`. El adapter lee ahora el `.provenance.json` hermano, avisa si no puede verificar y solo se salta con `ACE_STEP_SKIP_INTEGRITY=1`. **Coste, medido en GPU el mismo día:** recorrer los 7,5 GB por el bind mount solo para hashearlos costó **217 s** por arranque (primera versión, `113f99d`); por eso el hash se calcula ahora **de paso en la lectura contigua** (`cargar_contiguo(digestor=…)`, `855b00b`): 0 s adicionales en `weights_download`, y `vram_load` quedó en 118 s en la corrida de validación (el SHA-256 va en el mismo hilo que la lectura, ~7 GB a ~1 GB/s ≈ 7 s; el resto es estado de la caché de disco). Pendiente una medición controlada frente a los 72,9 s de referencia.
5. **`render()` no reentrante**: serializado con lock y aviso; `release()` toma el mismo lock. La concurrencia real de `T-04` exige procesos separados.
6. **Segunda revisión sobre los arreglos (mismo día):** la corrección del eje del limitador destapó que sus convoluciones pasaban a operar sobre la pista entera, **O(N·K): 205 s de CPU por 180 s de audio**, contados como `gpu_seconds`; y el `minimum` contra la ganancia cruda dejaba un **escalón de ~3 dB en una muestra** a ±10 ms de cada pico (un clic que G1 habría atribuido al modelo). Reescrito en O(N log W): mínimo deslizante por duplicación (anticipación 10 ms + retención 100 ms) y media móvil causal por suma acumulada (rampa lineal de ataque que termina en el pico, meseta, rampa de salida), con la garantía de techo demostrada sin `minimum`. 60 s de audio: de 69,6 s a 0,12 s; pendiente acotada por test; `tiempos["limiter_s"]` en el informe.
...
**Endurecimiento en tiempo de ejecución (2026-09-03, revisión, hallazgo I-3).** `generar.cmd` lanza ahora el contenedor con `--network none --read-only --tmpfs /tmp --tmpfs /home/runner:exec --cap-drop ALL --security-opt no-new-privileges --pids-limit 256`. Las variables `*_OFFLINE=1` de la imagen eran una petición a la librería, no una barrera, y el código vendorizado corría con salida a red. **Dos fallos medidos al validarlo en GPU y corregidos:** con `--read-only`, Triton (lo trae torch 2.13 y lo usa el detokenizer del planificador) no podía crear `~/.triton`; y con un tmpfs para el directorio personal, que Docker monta `noexec` por defecto, no podía cargar el `cuda_utils.so` que compila. **Compromiso registrado:** el directorio personal efímero admite ejecución, es decir, el aislamiento admite código compilado en tiempo de ejecución en ese directorio; la alternativa limpia (precompilar la caché de Triton en el build y montarla de solo lectura) queda como mejora. Validado con una generación real de 20 s (`revision-0903-20s-…wav`, código de salida 0). Pendiente de la misma revisión: pines de pip sin `--require-hashes` y `pyyaml` (usado por el prompt del planificador) sin fijar.
...
> |---|---|---|---|
> | ~~**P-01**~~ | ✅ **RESUELTA el 2026-09-03** — El veredicto se ata al **modelo y su adapter**, identificados por los `sha256` de las **fuentes upstream** del `.provenance.json`, no por el hash ni el dtype del artefacto fusionado: refundir los mismos pesos en bf16 para el servidor **no** es un adapter nuevo y **no** reabre el gate. Con asimetría: un `GO` en `tier3` se transfiere hacia arriba, un `NO-GO` **no**. | Decidido **antes** de generar, que es la única forma de que valga. Regla escrita en `gates/g1-protocolo.md` **§8.7** | ✔ cerrada |
...
| 2026-09-03 | **Cierre de la deuda que la revisión dejó señalada (fases 6 y 7).** 713 → 814 tests, cada punto con su rojo antes del código. **Corrección de un fallo real de niveles de GPU:** la cadena decía ser «idéntica a upstream» y redondeaba al GB antes de comparar, cuando upstream compara GB crudos con tolerancia (15,5 y 20,0); consecuencia medible, una tarjeta de 20 GB nominales (informa ~19,5) caía en `tier6b` —sin offload, lote 8, planificador de 4B— en vez de `tier6a`. **Presupuesto por defecto de 600 → 1800 s**, atado a la medición de `T-03` (la planificación sola de una pista de 240 s cuesta 615,3 s), porque el modo servidor con valores de fábrica abortaba cualquier pista larga. **`GenerationRequest` tipa sus campos** y acota la duración por cordura (3600 s, que NO es el techo del modelo: ese sigue en el shim, bajarlo aquí acoplaría el contrato). **Puerta de esquema movida dentro del bloque que limpia** (era la única vía de fallo de `build_pipeline` que devolvía al adapter un diccionario con los pesos dentro). **`bpm`/`keyscale`/`timesignature` tipados**. **Tests de los buffers no persistentes**, que `strict=True` no detecta. **`carga_contigua` levanta `ArtefactoIlegible` y no `KeyError`** ante dtypes desconocidos, porque desde esta revisión la clase de excepción decide si se cae al respaldo. **Candado de vendor con las guardias probadas sobre árboles fabricados**. **El auditor de opcodes de pickle pasa de un caso de prueba a 35**, uno por cada opcode letal, construidos como bytes y nunca deserializados; verificado por mutación que 9 de 9 fallarían si se desactivara la comprobación. **`_artefacto_declara_lm` cubierto** tras sus dos bugs. **Kit de G1: SHA-256 por pista** en registro, mapa y manifiesto, y **el ciego, que estaba roto**: el informe de la raíz de la carpeta de evaluación publicaba `semilla_maestra`, de la que se derivan todos los testigos, así que el índice de toma se reconstruía sin abrir nada sellado. **Licencias:** `NOTICE` en la raíz y `vendor/LICENSES/` con los tres textos; la licencia del código propio se deja sin declarar a propósito, es decisión del propietario. **`pyyaml` fijado**, que se ejecuta en el prompt del planificador y llegaba transitivo sin versión. | orquestador `/dev-cycle` |
| 2026-09-03 | **Triaje de las 34 firmas pendientes, y hoja de firmas preparada.** Conclusión central: **las firmas no son el cuello de botella**. Solo **dos** de las 34 necesitan un dato que hoy no existe —`A-13` (recuento de líneas base) y `B-15` (dictamen sobre MUSDB18)—; las otras 32 son decisiones puras o cosas que viven detrás de un gate. Se podrían firmar todas hoy y `G1` no se acercaría un día, porque lo que separa el gate son **las horas del propietario**: las 10 letras y las 10 líneas base. **Dos hallazgos verificados contra el repositorio:** (1) el repositorio era **público** y se llamaba `suno-sondo-clone`, un nombre que declara lo contrario de la primera regla de `CLAUDE.md` — **ya se ha privatizado** (comprobado: `visibility: PRIVATE`); queda el renombrado, que ya no es urgente. (2) **El protocolo de G1, ejecutado con fidelidad, producía una sesión inválida**: la regla anti-inflación de §3 rebajaba puntuaciones «al consolidar la hoja», §7.3 consolida *después* de romper el ciego, y §8.1 anula la sesión si una puntuación se modifica después de romperlo. **Corregido** en §3 y §3.6: la rebaja se aplica **al puntuar, con el ciego intacto**. Se crea [`gates/firmas-2026-09-03.md`](./gates/firmas-2026-09-03.md) con los cinco textos que **sí tienen ventana** redactados y razonados, a falta de decisión y fecha: `A-22` (renombrado), `A-19`=`B-21` (variante B, que además hace que la sesión quepa en las 2 h del propio protocolo), `A-10` (ratificar la corrección), `A-08` (reparto 6/4 de idiomas, que está aguas arriba de escribir las letras) y `§2.1-bis` (la tabla de precompromiso de líneas base). Queda escrito lo que **no** se firma y por qué. | orquestador `/dev-cycle` |
