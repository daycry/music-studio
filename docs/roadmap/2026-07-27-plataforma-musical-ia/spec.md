...
| D-06 | **ACE-Step 1.5 como modelo de referencia inicial** (**MIT** — no Apache 2.0: corregido el 2026-09-03 contra la evidencia de `T-06`, ver §11.1; **8 GB VRAM es el suelo con offloading, 24 GB la cifra de confort**), con **HeartMuLa como segundo adapter** y **YuE 7B como candidato condicional** (tercer adapter, **fuera de la Fase 1**) | ACE-Step es el mejor punto de partida local generalista, con iteración rápida y estilo dirigible; HeartMuLa cubre multilingüe y aporta HeartTranscriptor. **YuE no es un «adapter adicional» ya presupuestado**: es una partida condicional de Fase 2 (≈ 50 h con el router, `evaluation.md` §7 C-01) que solo se activa si G1-bis demuestra que la adherencia a la letra de los dos primeros modelos es insuficiente | MusicGen Stereo: licencia **CC BY-NC**, no comercial. Queda **descartado por licencia**. Presuponer tres adapters en Fase 1: contradice D-16 |
...
class ModelDescriptor(BaseModel):
    ...
    license: str                     # "MIT" (ACE-Step 1.5; Apache-2.0 es de ACE-Step v1 3.5B)
    commercial_use: bool
...
| S-05 | Los pesos de licencia permisiva (**MIT** en ACE-Step 1.5, **Apache 2.0** en HeartMuLa) pueden desplegarse en la infraestructura elegida sin restricción contractual adicional | Bloqueante |
...
|--------|--------|----------|-----------|-----------------|--------------|-----------|
| **ACE-Step 1.5** | 3,5B | **MIT** (†) | **8 GB (suelo, con offloading → más lento)** | **24 GB** (RTX 4090 / 5090: rendimiento «perfecto» según el usuario) | LM-planner + diffusion-renderer híbrido | Mejor punto de partida local generalista; iteración rápida, estilo dirigible |
...
| **🆕 ACE-Step 1.5 XL** (`xl-base`/`xl-sft`/`xl-turbo`, candidatas de spike) | 4B (DiT) | **MIT** (†) | ≥ 12 GB con offload | ≥ 20 GB | DiT | Mayor calidad de audio que la base 3,5B; candidatas a incluir en la medición del spike de Fase 0 (`T-03`/`T-05`) y en el gate G1 si la VRAM local lo permite |

**(†) Corrección de licencia 2026-09-03 — ACE-Step 1.5 es MIT, no Apache 2.0.** Hasta hoy esta tabla, D-06, S-05 y el «matiz legal» del final de §11.1 decían **Apache 2.0** para ACE-Step 1.5 y para la familia XL (`xl-base`/`xl-sft`/`xl-turbo`). Es falso: **`github.com/ace-step/ACE-Step-1.5` publica el proyecto bajo MIT** (sidebar «MIT license», README «This project is licensed under [MIT]») y el repo de Hugging Face `ACE-Step/Ace-Step1.5` declara el tag `license: mit`. Apache 2.0 es la licencia de **ACE-Step v1 3.5B**, que es **otro modelo** (`ACE-Step/ACE-Step-v1-3.5B`, tag `license: apache-2.0`): de ahí la confusión arrastrada desde julio 2026. Verificado en `T-06` ([`spikes/comparativa-modelos.md`](./spikes/comparativa-modelos.md) §4.1 y corrección #1 de su tabla de discrepancias), con copia archivada del fichero de licencia en `D:/srv/ace-step/provenance/LICENSE.acestep.mit.txt` (1.064 B, sha256 `05a6bce42a62636d2cfb24139cc008b6b899754e244175814bb5dd2f4a485357`).

**Por qué no es un detalle cosmético:** el manifiesto de procedencia registra la licencia de los pesos **en cada generación** (invariante innegociable de `CLAUDE.md`), y escribir «Apache 2.0» sobre unos pesos MIT sería **un dato falso en el artefacto que existe precisamente para ser auditable**. **MIT y Apache 2.0 son ambas permisivas y compatibles con uso comercial**, de modo que la corrección **no cambia ninguna cifra, fase, estado ni el veredicto de G2** — pero sí cambia lo que se escribe en el manifiesto y lo que se puso delante de legal (ver [`gates/g2-matriz-resultados.md`](./gates/g2-matriz-resultados.md) §0-bis). *Al margen de esta corrección, `T-06` documenta que los «3,5B» de la primera columna son de v1 — en la serie 1.5 la base es DiT 2B y la XL DiT 4B —; no se toca aquí porque este cambio se limita a licencias.*

**⚠️ Licencia sin verificar — DiffRhythm 2.** Su celda (**Apache 2.0**) **no está verificada contra fuente primaria**: `T-06` dejó el modelo explícitamente fuera de la comparativa (no está en D-06 y no tiene tarea asignada), así que el dato procede de la investigación de julio 2026 y nadie lo ha comprobado. **Se deja intacto a propósito**: cambiarlo sin evidencia repetiría, en sentido contrario, el error de ACE-Step. **Antes de integrar el modelo hay que verificar la licencia** y archivar la copia, como exige el invariante de licencias de `CLAUDE.md` (anotado en I-13b). Las licencias de **HeartMuLa** y **YuE 7B** (Apache 2.0) y la **MiniMax-Music3 Community License** sí están verificadas y **no se tocan**.
...
| 2026-09-03 | **Corrección de licencia: ACE-Step 1.5 es MIT, no Apache 2.0.** El dato era falso en cinco puntos de este documento y se arrastraba desde la investigación de julio 2026 por **confusión con ACE-Step v1 3.5B**, que sí es Apache 2.0 y es **otro modelo**. Corregidos: **D-06**; el ejemplo de `ModelDescriptor.license` de §7 (su `id` es `"ace-step"` y su `version` `"1.5.0"`, luego el valor se leía como afirmación sobre ACE-Step y no como marcador genérico); **S-05** (reescrito a «pesos de licencia permisiva» nombrando la licencia por modelo); las filas **ACE-Step 1.5** y **ACE-Step 1.5 XL** de la tabla de §11.1; y el **matiz legal** que cierra §11.1. Evidencia: `T-06` ([`spikes/comparativa-modelos.md`](./spikes/comparativa-modelos.md) §4.1 y corrección #1 de su tabla de discrepancias) y copia archivada `LICENSE.acestep.mit.txt` (sha256 `05a6bce4…a485357`). **Por qué:** el manifiesto de procedencia registra la licencia de los pesos en cada generación, y «Apache 2.0» sobre pesos MIT sería un dato falso en el artefacto que existe para ser auditable. **No se tocan** HeartMuLa ni YuE 7B (Apache 2.0 verificado) ni MiniMax-Music3 (licencia comunitaria propia; sus dos menciones de «no Apache 2.0» siguen siendo ciertas); **DiffRhythm 2 se deja intacta** y se añade nota de **licencia no verificada**. **Sin efecto en cifras, fases, estados ni gates**: MIT y Apache 2.0 son ambas permisivas y admiten uso comercial. | dev-cycle (orquestador) |
